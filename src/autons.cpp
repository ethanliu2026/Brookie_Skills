...
void skills() {
  ...
  chassis.turn_to_point(22.3, -22,0);
  chassis.drive_to_point(22.3, -22., 0, 8, 6, 1, 500, 650); //drive to three pile, -27
  chassis.turn_to_point(11., -11.7, 0, 5, 0.5, 350, 500); //turn to low goal
  straightline_to_pose(11., -11.7, 12, 3, 0.5, 450, 650); //drive to low goal
  intake_lift = true;
  Intake.spin(forward);
  chassis.drive_distance(1.05, 0, 3, 0, 2, 80, 100);
  ...
  intake_lift = false;
  chassis.turn_to_point(42.7, -45.3, 0, 7, 5, 400, 520);
  matchloader_down = true;
  Intake.spin(forward, 12.7, volt);
  straightline_to_pose(42.7, -45.3, 8, 10, 0.1, 550, 650); //drive to matchloader 1
  ...
  chassis.drive_distance(17, 180, 10, 6, 3, 350, 400);

  hood_down = true;
  chassis.drive_distance(12, 180, 3, 6, 3, 800, 1200); //stay to intake
  chassis.drive_distance(-10, 180, 10, 6, 3, 300, 350);
  matchloader_down = false;
  chassis.drive_to_point(55.8, -30.7, 0, 5, 6, 7, 500, 650); //back to right of goal
  chassis.turn_to_angle(180, 8, 1, 300, 380);
  straightline_to_pose(55.8, 30.7, 10, 0, 1, 800, 920, true);
  horizontalResetPosition();
  ...
  matchloader_down = true;
  //chassis.turn_to_angle(0);
  straightline_to_pose(48.5, 56, 7, 6, 1, 400, 500); //drive to first matchloader
  //chassis.drive_to_point(49.2, 45, 6, 8, 6, 3, 350, 400);
  //chassis.drive_distance(36, -3, 8, 6, 3, 350, 400);

  hood_down = true;
  Intake.spin(forward, 12.7, volt);
  chassis.drive_distance(15, 1, 3, 6, 3, 800, 1400); //stay to intake, originally 15
  ...
  chassis.turn_to_angle(180); //distance reset turn
  resetPositionSkills();
  ...
  straightline_to_pose(-47.5, 56, 7, 6, 0.1, 400, 500); //drive to second matchloader
  ...
  chassis.turn_to_point(-45., -39, 180, 8, 1, 300, 340);
  straightline_to_pose(-45., -39, 8, 8, 1, 400, 500, true); 
  
  
  matchloader_down = true;
  
  chassis.turn_to_angle(180, 7, 1, 300, 500);
  straightline_to_pose(-44., -56, 6, 1, 1, 500, 600); //approach matchloader 4
  
  
  hood_down = true;
  Intake.spin(forward, 12.7, volt);
  chassis.drive_distance(10, 0, 3, 0, 1, 800, 1400); //stay to intake
  straightline_to_pose(-45., -18, 7, 1, 1, 500, 600, true); //approach long goal 1st time
  //chassis.drive_distance(-21.5, 0, 7, 7, 1, 500, 600); //back into goal
  hood_down = false;
  wait(700, msec);
  matchloader_down = false;
  //chassis.drive_distance(15, 0, 12, 0, 1, 300, 400);
  chassis.drive_to_point(-20, -55, 12, 12.5, 7, 5, 500, 600);
  chassis.drive_to_point(0, -67, 10.5, 12.5, 6.5, 1, 400, 500);
