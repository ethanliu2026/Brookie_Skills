...
void skills() {
  ...
  straightline_to_pose(58, 20, 10, 0, 1, 800, 1000, true);
  //chassis.drive_to_point(45.2, 36, 4, 5, 6, 7, 800, 1000);
  chassis.turn_to_point(47, 39, 180, 8, 1, 300, 400); //turn to goal 1 alignment point
  straightline_to_pose(47, 39, 8, 8, 1, 400, 500, true);
  chassis.turn_to_angle(0, 7, 1, 300, 500);
  straightline_to_pose(47, 18, 6, 1, 1, 500, 600, true); //approach long goal 1st time
  ...
  resetPositionSkills();
  chassis.turn_to_point(-46.4, 40, 0, 8, 1, 300, 480); //turn to matchloader 2
  straightline_to_pose(-46.4, 40, 8, 8, 0.1, 900, 1000);
  ...
  hood_down = true;
  Intake.spin(forward, 12.7, volt);
  chassis.drive_distance(20, 1, 183, 6, 3, 800, 1400); //stay to intake, originally 15
  straightline_to_pose(48, 18, 6, 1, 1, 500, 600, true); //approach long goal 2nd time
  chassis.drive_distance(-6, 0, 5, 0, 1, 100, 150); 
  hood_down = false;
  //autoResetPosition();
